...
pub fn enrich(
    ...
    const conn = pool.acquire(allocator, language_support_val.language, &lsp_config, root, log) catch {
        log.warn("LSP: failed to acquire connection", &.{Field.string("server", lsp_config.server_name)});
        return .{};
    };
    defer pool.release(language_support_val.language);

    var warmup_elapsed: u64 = 0;
    if (conn.openedFileCount() == 0) {
        conn.openAllFiles(allocator, graph, lang_id, root, log);

        const warmup = conn.client.drainNotifications(allocator, options.warmup_timeout_ms, log);
    ...
    var result = try enrich_fn(allocator, graph, &conn.client, wl, log);
...
test "enricher handles no-op cases" {
    ...
    const result = try enrich(std.testing.allocator, &graph, &no_lsp, &wl, &pool, .{});

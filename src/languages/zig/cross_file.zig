...
/// identifier segments extracted from post-import field accesses.
...
/// Maximum depth for field_expression chains.
/// 16 covers realistic qualified paths while bounding stack usage.
...
pub fn resolveQualifiedCall(
    ...
    for (chain, 0..) |segment, seg_idx| {
        ...
        // If no direct child matched (var-tracked chains that skip the
        // intermediate type), search all descendants via scope_index.
        // Only resolve when exactly one matches.
        if (matched_id == null) {
            matched_id = scope_index.findUniqueDescendant(g.nodes.items, current_scope_id, segment);
        }

        // Self refers to the current type or file-struct; stay at scope.

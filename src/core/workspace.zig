...
const binary = @import("../storage/binary.zig");
...
/// from `<workspace_dir>/<project.path>/.zcodeprism/graph.bin`, and
/// return the assembled `Graph`. Caller deinits with the same allocator.
/// `project_ranges` are freed internally.
pub fn loadAndAssemble(
    ...
    const ws = try parseWorkspaceConfig(allocator, content_z, ws_dir);
    defer freeWorkspace(allocator, &ws);

    try validateWorkspace(io, &ws, ws_dir);
    ...
    const assembled = try assembleWorkspace(allocator, &ws, project_graphs);
